...
import { Text, TouchableOpacity, View, SafeAreaView, FlatList, StyleSheet, SectionList, Modal, TextInput, Keyboard, Image } from 'react-native';
  ...
  if (loaded === false) {
    ...
    return (
      <View style={{ flex: 1, alignItems: 'center' }}>
        ...
        >
          <SafeAreaView style={{ alignItems: 'center', flex: 0.1, backgroundColor: 'white', borderBottomWidth: 1, borderColor: '#D5D5D5' }}>
            <View style={{ width: '90%', flexDirection: 'row', justifyContent: 'center', alignItems: 'center', position:'absolute', bottom:0 }}>
              <FontAwesome name="search" size={20} style={{ width: '5%', color: '#8A8A8A' }} />
              <TextInput
              ...
              />
              <TouchableOpacity onPress={closeModalSearchFromCancel}>
                <Text> Cancel</Text>
              </TouchableOpacity>
            </View>
          </SafeAreaView>
          <TouchableOpacity style={{ flex: 1, backgroundColor: "rgba(60,60,60,0.85)" }} onPress={() => { Keyboard.dismiss(); setModalVisible(false) }}>
            {showSearch === true ? (
              <FlatList
                ...
                renderItem={({ item }) => {
                  return (
                    <TouchableOpacity key={item.id} style={styles.btnMenu} activeOpacity={0.9} onPress={() => navigateToItemFromModal(item)}>
        ...
        <View style={{ backgroundColor: 'white', flexDirection: 'row' }}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              return (
                <TouchableOpacity key={item.id} style={{ borderBottomWidth: click === item.id ? (2) : (0) }} onPress={() => scrollToSection(item.id)}>
        ...
        <View style={{ paddingTop: 10, height: '100%', flex: 1, width: '100%' }}>
          <SectionList
            ref={scrollToItem}
            onViewableItemsChanged={tapBar == false ? (onViewableItemsChanged) : (null)}
            viewabilityConfig={{ itemVisiblePercentThreshold: 95, waitForInteraction: true }}
            sections={menu}
            ...
            renderSectionHeader={({ section: { title } }) => {
              return (
                <Text style={{ fontSize: 23, fontWeight: 'bold', padding: 20, backgroundColor: 'white' }}>{title}</Text>
              )
            }}
          />
        </View>
