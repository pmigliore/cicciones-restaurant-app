...
import { linkWithCredential, PhoneAuthProvider, signInWithCredential, EmailAuthProvider} from 'firebase/auth';
import { doc, setDoc } from 'firebase/firestore';

const API_URL = 'https://us-central1-restaurantapptemplate-691c3.cloudfunctions.net/addCustomerToStripe'
...
export default function VerifyPhoneNumber({ route }) {
    ...
    const { verificationId, email, password, phoneNumber, firstName, lastName } = route.params;
            ...
            .then((result) => {
                linkWithCredential(auth.currentUser, credentialEmail)
                    .then( async (result) => {
                            ...
                            const setInformation = doc(db, 'clients', 'user', 'Information', auth.currentUser.uid)
    ...
    return(
            <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
                <View style={styles.container}>
                    ...
                    <CustomButton
                        buttonTxt={loaded == true ? 'Continue' : <ActivityIndicator color='white' />}
